...
void print_field (char *base, char *name, int sub, int n) {
  const char *type = c_type (base);
  if (type) print ("  %s %s", type, name);
  else if (sub) print ("  SubstitutionType %s", base, name);
...
void print_particle (Particle *part, SchemaType *head) {
  Particle *p; Field *f; SchemaType *t; int sub;
  foreach (p, part) {
    switch (p->kind) {
    case PartElement:
      f = &p->field;
      t = find_by_name (head, f->type);
      sub = t? t->substitutable : 0;
      if (p->max < 0)
	print ("  List *%s;\n", f->name);
      else if (!is_boolean (f->prim))
	print_field (f->type, f->name, sub, p->max);
      break;
    case PartGroup:
    case PartChoice:
    case PartSequence:
...
int is_derived (SchemaType *u, SchemaType *t, SchemaType *head) {
  while (u && u->base) {
    u = find_by_name (head, u->base);
    if (u == t) return 1;
  } return 0;
}
...
int xs_primitive (char *name, SchemaType *head) {
  ...
  while ((index = string_index (name, xs_names, 12)) == 12) {
    ...
    if (t->attr || t->part || !t->base) return 0;
    name = t->base;
    if (!n && t->maxLength > 0) n = t->maxLength;
  }
  return ST_SIMPLE | xs_type (index+1, n);
}
...
void part_flags (SchemaType *t, Particle *part, SchemaType *head) {
  ...
  foreach (p, part) {
    ...
    case PartElement: f = &p->field;
      f->prim = xs_primitive (f->type, head);
      if (!is_pointer (f->prim)) {
	bool = is_boolean (f->prim) && p->max > 0? p->max : 0;
...
// return list of types on the path from a set of types to the lca type
List *lca_path (SchemaType *lca, List *types, SchemaType *head) {
  List *a = type_ancestors (lca, head), *path = NULL, *l, *anc;
  foreach (l, types) {
    anc = type_ancestors (l->data, head);
    path = list_union (path, list_subtract (anc, a));
  } return path;
}
...
List *pre_flag_groups (List *groups, SchemaType *head) {
  List *l, *m, *n, *post = NULL, *anc; FlagGroup *g;
  foreach (l, groups) { g = l->data;
...
int get_base_bits (SchemaType *t, SchemaType *u, SchemaType *head) {
  int base = 0; List *l; Flag *f;
  if (u->base && (u = find_by_name (head, u->base))) {
    if (u->flags) return u->base_bits + u->bits;
    foreach (l, u->pre) { f = l->data;
      if (find_by_data (f->types, t))
	base = f->bit + f->count + f->bool;
    }
    return base? base : get_base_bits (t, u, head);
  }
  foreach (l, global_flags) { f = l->data;
    if (find_by_data (f->types, t))
      base = f->bit + f->count + f->bool;
...
void compute_flags (List *sorted, SchemaType *head) {
  ...
  // distribute flags to types or flag groups
  foreach (f, flags) {
  ...
  foreach (l, sorted) { t = l->data;
    ...
    t->base_bits = get_base_bits (t, t, head);
...
void print_xs_type (int type) { int n;
  ...
  type ^= ST_SIMPLE; n = type >> 4; type &= 0xf;
...
void print_schema_entry (SchemaEntry *se, char *parent,
			 char *name, char *type, SchemaType *head) {
  int bool = is_boolean (se->type);
  print ("  {");
  if (parent && !bool)
    print (".offset=offsetof(SE_%s_t,%s), ", parent, name);
  print (".min=%d, ", se->min);
  if (se->max) print (".max=%d, ", se->max);
  if (se->unbounded) print (".unbounded=1, ");
  if (se->attribute) print (".attribute=1, ");
  if (se->st) print (".st=1, ");
  if ((se->min < se->max && !is_pointer (se->type)) || bool)
    print (".bit=%d, ", se->bit);
  if (se->type & ST_SIMPLE) {
    print (".type=ST_SIMPLE|"); print_xs_type (se->type);
...
List *local_names = NULL;

void insert_local_name (char *name) {
  if (!find_by_name (local_names, name)) {
    List *n = list_insert (NULL, name);
    local_names = insert_sorted (local_names, n, compare_names);
  }
}
...
TableEntry *gen_type_part (Particle *part, SchemaType *head) {
  TableEntry *entries = NULL; SchemaEntry *se; SchemaType *t;
  Particle *p; Field *f;
  foreach (p, part) {
    switch (p->kind) {
    case PartElement: f = &p->field;
      t = find_by_name (head, f->type);
      se = type_alloc (SchemaEntry);
      se->st = t? t->substitutable : 0;
      se->min = p->min;
      if (f->flag) se->bit = f->flag->bit;
      if (p->max < 0) se->unbounded = 1;
      else se->max = p->max;
      if (f->prim > 0) se->type = f->prim;
    ...
    case PartChoice:
    case PartSequence:
      entries = list_cat (gen_type_part (p->child, head), entries);
    case PartAny:
      break;
    }
  }
...
TableEntry *gen_type_parts (SchemaType *t, SchemaType *head) {
  TableEntry *entries = NULL;
  if (t->base) {
    SchemaType *u = find_by_name (head, t->base);
    entries = list_cat (gen_type_parts (u, head), entries);
  }
  if (!t->restriction)
    entries = list_cat (gen_type_part (t->part, head), entries);
  return entries;
}

TableEntry *gen_type_entries (SchemaType *t, SchemaType *head) {
  TableEntry *entries = NULL;
  List *list = collect_attr (t, head), *l;
  foreach (l, list) {
    AttrDecl *a = l->data; Field *f = &a->field;
    SchemaEntry *se = type_alloc (SchemaEntry);
    se->type = f->prim;
  ...
  return list_reverse (list_cat (gen_type_parts (t, head), entries));
}

void gen_schema_entries (List *sorted, int index, SchemaType *schema) {
  List *l;
  foreach (l, sorted) {
    SchemaType *t = l->data;
    if (t->name) insert_local_name (t->name);
    if (t->kind == ComplexType) {
      t->index = index;
      t->entries = gen_type_entries (t, schema);
      if (t->name) index++;
...
void print_schema (List *sorted, SchemaDoc *doc) {
  ...
  TableEntry *te; List *l, *s; ElementDecl *e;
  gen_schema_entries (sorted, length, doc->types);
  print ("// auto-generated by schema_gen\n\n");
  print ("const SchemaEntry se_entries[] = {\n");
  foreach (e, doc->elements) {
    int prim = xs_primitive (e->type, doc->types);
    print ("  {.min=1, .max=1, ");
    if (prim > 0) {
      print (".type=ST_SIMPLE|");
      print_xs_type (prim);
    } else {
      print (".index=%d", get_index (doc->types, e->type));
    }
    print ("}, // %s\n", e->name);
    insert_local_name (e->name);
  }
  foreach (s, sorted) {
    SchemaType *t = s->data;
    if (t->kind != ComplexType) continue;
    ...
    foreach (te, t->entries) {
      SchemaEntry *se = te->se;
      se->n = productions (te);
      print_schema_entry (se, t->name, te->name, te->type, doc->types);
    }
  }
  print ("  {0}\n};\n\n");
  print ("const char * const se_elements[] = {");
  foreach (e, doc->elements) print ("\"%s\", ", e->name);
  print ("};\n\n");
  print ("const uint16_t se_ids[] = {");
  foreach (s, sorted) {
    SchemaType *t = s->data;
    if (t->kind != ComplexType) continue;
    if (t->name) print ("%d, ", find_index_by_name (local_names, t->name));
    foreach (te, t->entries)
      print ("%d, ", find_index_by_name (local_names, te->name));
  }
  print ("};\n\n");
  print ("const char * const se_names[] = {");
  foreach (l, local_names) print ("\"%s\", ", l->data);
  print ("};\n\n");
  print ("const uint16_t se_types[] = {");
  foreach (l, local_names) {
    SchemaType *t = find_by_name (doc->types, l->data);
    if (t) { int prim;
      if (prim = xs_primitive (t->name, doc->types)) {
	print_xs_type (prim); print ("|ST_SIMPLE, ");
      } else print ("%d, ", t->index);
    } else print ("0, ");
  }
  print ("};\n\n");
  print ("Schema se_schema = "
	 "{\"%s\", \"S1\", %d, %d, se_names, se_types, se_entries, se_elements, se_ids};\n",
	 doc->targetNamespace, length, list_length (local_names));
}

void print_symbols (SchemaDoc *doc) {
  ElementDecl *e; SchemaType *t; int i = 0;
  foreach (e, doc->elements) 
    print ("#define SE_%s %d\n", e->name, i++);
  foreach (t, doc->types) { int prim;
    print ("#define SE_%s_type ", t->name);
    if (prim = xs_primitive (t->name, doc->types)) {
      print ("("); print_xs_type (prim); print ("|ST_SIMPLE)\n");
    } else print ("%d\n", t->index);
  }
  print ("\n");
}
